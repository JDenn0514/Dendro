...
- **Licence.** The site states one licence for all its content: "licensed under a Creative Commons Attribution-ShareAlike 4.0" (`/about/licence/`, read 2026-09-25). No caption states a licence of its own. The per-image check reads the caption:
  ...
  - A caption with `©`, `(c)`, "permission", "courtesy", or "rights reserved" (any case) is skipped, because its rights may differ from the site licence. The old `harvest.cjs` applies the same rule, and the manifest's 19 TSO rows passed it.
  - A caption with no `Image <Name>` credit is skipped. `<Name>` is the text after the last `Image `. If that text holds a digit, or a word that starts with a lower-case letter and is not a name joiner (`and`, `&`, `de`, `del`, `della`, `der`, `den`, `di`, `da`, `do`, `dos`, `du`, `la`, `le`, `van`, `von`, `y`), the caption goes on after the name, and it is skipped.
